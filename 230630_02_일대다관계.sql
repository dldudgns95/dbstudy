...
CREATE TABLE STUDENT_T(
    ...
    SCH_CODE NUMBER ,                        --외래키는 참조 타입과 같은 타입이여야 함
    STU_NAME VARCHAR2(10) NOT NULL,          --SCH_CODE는 SCHOOL_T 테이블의 SCH_CODE를 참조한다.
    CONSTRAINT PK_STU PRIMARY KEY(STU_NO),   --제약조건의 이름은 PK_STU, STU_NO에 PRIMARY KEY 지정
    CONSTRAINT FK_SCH_STU FOREIGN KEY(SCH_CODE) REFERENCES SCHOOL_T(SCH_CODE) ON DELETE CASCADE  --제약조건의 이름은 FK_SCH_STU, SCH_CODE에 FOREIGN KEY 지정
);
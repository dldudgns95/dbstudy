...
SELECT SYSTIMESTAMP - TO_TIMESTAMP('23/07/01', 'YY/MM/DD'),                     --경과한 기간이 TIMESTAMP 형식으로 반환
       EXTRACT(DAY FROM SYSTIMESTAMP - TO_TIMESTAMP('23/07/01', 'YY/MM/DD'))    --경과한 일수에서 DAY만 추출
  FROM DUAL;
...
--8. 경과한 개월 수 구하기
SELECT MONTHS_BETWEEN(SYSDATE, TO_DATE('23/01/01', 'YY/DD/MM'))
  FROM DUAL;
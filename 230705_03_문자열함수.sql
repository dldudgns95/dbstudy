...
SELECT EMAIL
 ...
 --7. 바꾸기
 SELECT EMAIL
      , REPLACE(EMAIL,'A','$')      --EMAIL에 있는 모든 'A'를 '$'로 바꾸기
   FROM EMPLOYEES;
...
SELECT JOB_ID
     , CONCAT(CONCAT(SUBSTR(JOB_ID,1,INSTR(JOB_ID,'_')-1),' / '), SUBSTR(JOB_ID,INSTR(JOB_ID,'_')+1,LENGTH(JOB_ID) - INSTR(JOB_ID,'_')))
...
--      EX) FIRST_NAME : Steven
--          LAST_NAME  : King
--          FULL_NAME  : STEVEN KING
SELECT FIRST_NAME
     , LAST_NAME
     , CONCAT(CONCAT(UPPER(FIRST_NAME),' '),UPPER(LAST_NAME))
  FROM EMPLOYEES

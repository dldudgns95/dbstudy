...
SELECT DISTINCT MANAGER_ID
  FROM EMPLOYEES
 WHERE SUBSTR(JOB_ID,INSTR(JOB_ID,'_')+1) = 'MAN';
...
SELECT SUBSTR(SALARY,1,MOD(LENGTH(SALARY),3))  || ',' || SUBSTR(SALARY,MOD(LENGTH(SALARY),3)+1,3)
  FROM EMPLOYEES
...
SELECT CEIL(MONTHS_BETWEEN(SYSDATE,TO_DATE(HIRE_DATE,'YY/MM/DD')))
     , HIRE_DATE
     , SYSDATE
  FROM EMPLOYEES;
...
SELECT  CASE
            WHEN CEIL(MONTHS_BETWEEN(SYSDATE,TO_DATE(HIRE_DATE,'YY/MM/DD'))) >= 240 THEN '퇴직금정산대상'
        ELSE ' '
       END

...
--사원 테이블과 부서 테이블에 모두 존재하는 부서번호 조회하기 (사원들이 근무 중인 부서번호만 조회하기)
SELECT DEPARTMENT_ID
  FROM DEPARTMENTS
INTERSECT
SELECT DEPARTMENT_ID
...
-- 부서 테이블에 존재하지만 사원 테이블에 존재하지 않는 부서 번호 조회하기
SELECT DEPARTMENT_ID
  FROM DEPARTMENTS
 MINUS
SELECT DEPARTMENT_ID
...
SELECT LVL, EMPLOYEE_ID, FIRST_NAME, LAST_NAME, MANAGER_ID FROM MY_SUBQUERY;

...
CREATE SEQUENCE CUST_SEQ START WITH 1000 ORDER;
...
INSERT INTO CUSTOMER_T(CUST_ID, CUST_NAME, CUST_ADDR) VALUES(CUST_SEQ.NEXTVAL, '박세리', '대한민국');
COMMIT;
...
/*
INSERT INTO ORDER_T(ORDER_ID, CUST_ID, BOOK_ID, AMOUNT) VALUES('200701'||'-'||ORDER_SEQ.NEXTVAL, 1000, 1,  1);
INSERT INTO ORDER_T(ORDER_ID, CUST_ID, BOOK_ID, AMOUNT) VALUES('200703'||'-'||ORDER_SEQ.NEXTVAL, 1000, 3,  2);
INSERT INTO ORDER_T(ORDER_ID, CUST_ID, BOOK_ID, AMOUNT) VALUES('200703'||'-'||ORDER_SEQ.NEXTVAL, 1001, 5,  1);
INSERT INTO ORDER_T(ORDER_ID, CUST_ID, BOOK_ID, AMOUNT) VALUES('200704'||'-'||ORDER_SEQ.NEXTVAL, 1002, 6,  2);
INSERT INTO ORDER_T(ORDER_ID, CUST_ID, BOOK_ID, AMOUNT) VALUES('200705'||'-'||ORDER_SEQ.NEXTVAL, 1003, 7,  3);
INSERT INTO ORDER_T(ORDER_ID, CUST_ID, BOOK_ID, AMOUNT) VALUES('200706'||'-'||ORDER_SEQ.NEXTVAL, 1000, 2,  5);
INSERT INTO ORDER_T(ORDER_ID, CUST_ID, BOOK_ID, AMOUNT) VALUES('200707'||'-'||ORDER_SEQ.NEXTVAL, 1003, 8,  2);
INSERT INTO ORDER_T(ORDER_ID, CUST_ID, BOOK_ID, AMOUNT) VALUES('200708'||'-'||ORDER_SEQ.NEXTVAL, 1002, 10, 2);
INSERT INTO ORDER_T(ORDER_ID, CUST_ID, BOOK_ID, AMOUNT) VALUES('200709'||'-'||ORDER_SEQ.NEXTVAL, 1001, 10, 1);
INSERT INTO ORDER_T(ORDER_ID, CUST_ID, BOOK_ID, AMOUNT) VALUES('200710'||'-'||ORDER_SEQ.NEXTVAL, 1002, 6,  4);

UPDATE ORDER_T
   SET ORDER_DT = TO_DATE(SUBSTR(ORDER_ID,1,6),'YY/MM/DD')
...
-- 가격 별로 순위를 정하여 1위인 책을 조회하시오.                  
SELECT BOOK_ID, BOOK_NAME, PUBLISHER, PRICE, RANKING
  FROM (SELECT BOOK_ID, BOOK_NAME, PUBLISHER, PRICE, RANK() OVER(ORDER BY PRICE DESC) AS RANKING
          FROM BOOK_T)
 WHERE RANKING = 1;
...
SELECT O.ORDER_ID   AS 주문번호
 ...
 WHERE ORDER_DT BETWEEN TO_DATE('20/07/05','YY/MM/DD') AND TO_DATE('20/07/09','YY/MM/DD')
 ORDER BY ORDER_DT; 
...
SELECT C.CUST_NAME              AS 고객명
     , SUM(B.PRICE * O.AMOUNT)  AS 총구매액
     , RANK() OVER(ORDER BY SUM(B.PRICE * O.AMOUNT) DESC) AS 순위
  FROM BOOK_T B INNER JOIN ORDER_T O
    ON B.BOOK_ID = O.BOOK_ID INNER JOIN CUSTOMER_T C
    ON C.CUST_ID = O.CUST_ID
 GROUP BY C.CUST_NAME, C.CUST_ID;
 ...
 SELECT A.고객명, A.총구매액
  FROM (SELECT C.CUST_NAME              AS 고객명
          ...
          FROM BOOK_T B INNER JOIN ORDER_T O
            ON B.BOOK_ID = O.BOOK_ID INNER JOIN CUSTOMER_T C
            ON C.CUST_ID = O.CUST_ID
         GROUP BY C.CUST_NAME, C.CUST_ID) A
 WHERE A.순위 BETWEEN 2 AND 3;
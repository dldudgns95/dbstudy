...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('나이는 ' || AGE  || '살 입니다.');
END;
...
--2. 참조 변수를 이용한 IF문 처리하기
DECLARE
    EMPLOYEEID EMPLOYEES.EMPLOYEE_ID%TYPE;
    SAL        EMPLOYEES.SALARY%TYPE;
BEGIN
    EMPLOYEEID := 140;  -- 임의의 사원번호 저장
    SELECT SALARY
      INTO SAL
      FROM EMPLOYEES
...
BEGIN
    ...
    CASE
        ...
        WHEN PHONENUMBER = '650' THEN
            MESSAGE := 'NORTH';
        END CASE;
    DBMS_OUTPUT.PUT_LINE(MESSAGE || ' 지역');
END;
...
-- 사원 번호가 100 ~ 109 사이의 사원번호, 사원명 출력하기
DECLARE
    EMPLOYEEID EMPLOYEES.EMPLOYEE_ID%TYPE;
    FIRSTNAME EMPLOYEES.FIRST_NAME%TYPE;
    LASTNAME EMPLOYEES.LAST_NAME%TYPE;
...
/*
    ...
    CONTINUE : 반복문의 시작부터 다시 실행하기 (특정한 값을 제외할 때 사용)
*/
...
BEGIN
    ...
    WHILE TRUE LOOP
        ...
        IF TOTAL > 100 THEN
            EXIT;
        END IF;
        -- 3으로 나눈 나머지를 MODULAR 변수에 저장
        SELECT MOD(N, 3) INTO MODULAR
          FROM DUAL;
        -- 3의 배수는 제외
        IF MODULAR = 0 THEN
            CONTINUE;   -- WHILE TRUE LOOP문의 첫 실행문으로 되돌아가서 실행한다.
        END IF;
        TOTAL := TOTAL + N;
    END LOOP;
    --결과 확인
...
EXCEPTION
    WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE(SQLCODE);  -- 예외 코드를 확인
        DBMS_OUTPUT.PUT_LINE(SQLERRM);  -- 예외 메세지
END;
